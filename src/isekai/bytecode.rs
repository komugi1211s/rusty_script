use super::parse::{BlockData, DeclarationData, Expr, FunctionData, ParserNode, Statement};
use super::token::{Token, TokenType};
use super::types::{toVmByte, Constant, OpCode, Type, Value};
...
impl Code 
{
    ...
    pub fn disassemble(&self, start: usize, max_len: usize) -> Vec<String> {
        ...
        while current < max
        {
            ...
            let padding = self.disassemble_pad(OpCode::from_u8(opbyte).unwrap_or(OpCode::Interrupt));
            if 0 < padding {
                current += 1;
        ...
        let bytecode_hex: Vec<String> = (&self.bytes[start .. max]).iter()
...
impl BytecodeGenerator {
    ...
    pub fn write_const(&mut self, constant: Constant, line: usize) -> usize {
        ...
        self.const_table.types.insert(start_index, constant.ctype.clone());
    ...
    fn handle_stmt(&mut self, data: Statement, line: usize) -> StatementHandleResult {
        ...
        match data {
            ...
            Statement::If(expr, if_block, optional_else_block) => {
                ...
                self.code.push_operands(usize::max_value().to_vm_byte(), line);
                ...
                self.code.rewrite_operands(end_of_if_block.to_vm_byte(), jump_opcode_index);
                ...
                if let Some(else_block) = optional_else_block {
                    ...
                    let after_operand = self.code.push_operands(usize::max_value().to_vm_byte(), line);

                    // Ifブロックの終了アドレスがあった部分を、Else避けJump分を加味して調整
                    self.code.rewrite_operands(after_operand.to_vm_byte(), jump_opcode_index);

                    // Ifブロックが丁度終わる位置のJumpオペランドを、Elseブロックの終了アドレスで上書き
                    let end_of_else_block = self.handle_stmt(*else_block, line).index;
                    self.code.rewrite_operands(end_of_else_block.to_vm_byte(), jump_block);
                }
            ...
            Statement::While(expr, while_block) => {
                ...
                let before_loop = self.code.current_length();
                let jump_conditional = self.code.push_opcode(OpCode::Jump, line);
                let after_jump_conditional = self.code.push_operands(usize::max_value().to_vm_byte(), line);
                ...
                let end_of_loop = self.code.push_operands(after_jump_conditional.to_vm_byte(), line);

                for i in self.break_call.drain(..)
                {
                    self.code.rewrite_operands(end_of_loop.to_vm_byte(), i);
                }
                self.code.rewrite_operands(before_expr.to_vm_byte(), jump_conditional);
            ...
            Statement::Continue => {
                if self.last_loop_start != usize::max_value() {
                    let break_index = self.code.push_opcode(OpCode::Jump, line);
                    self.code.push_operands(self.last_loop_start.to_vm_byte(), line);
                }
                handled_result.index = self.code.current_length();
                handled_result
            }
        ...
        match expr {
            ...
            Expr::Assign(name, expr) => {
                ...
                else
                {
                    ...
                    {
                        panic!("Type Mismatch when Assigning: {:?} != {:?}", declared_type, another_result._type);
                    }
                    ...
                    handled_result.index = self.code.push_operands((index as u16).to_vm_byte(), line);
                }
                handled_result
            }
            Expr::FunctionCall(func_name, open_paren, mut arguments) => {
                if let Expr::Variable(name) = *func_name {
                    ...
                    self.code.rewrite_operands(jump_here.to_vm_byte(), address_section);
