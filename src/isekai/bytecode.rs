...
impl ConstantTables 
{
    ...
    {
        let mut start_at = self.bytes.len();
    ...
    pub fn read_data_8(&self, index: usize) -> (u8, Type) {
        ...
        let _type = self.types.get(&index).unwrap().clone();
        (self.bytes[index], _type)
    }
...
impl ByteCode {
    ...
    pub fn disassemble(&self, start: usize, max_len: usize) -> Vec<String> {
        ...
        let bytecode_hex: Vec<String> = (&self.code_chunk[start..max])
            .iter()
            .map(|x| format!("{:02x}", x))
            .collect();
...
impl CodeGenerator {
    pub fn new() -> Self {
        Self {
            ...
            constants: ConstantTables::default(),
    ...
    pub fn traverse_ast(mut self, ast: Vec<ParserNode>) -> Result<(ByteCode, ConstantTables), ()> {
        for i in ast {
            self.handle_data(i);
        }
        self.constants.push_data(usize::max_value().to_vm_byte());
        Ok((self.chunk, self.constants))
    }
    ...
    pub fn write_const(&mut self, constant: Constant, line: usize) -> usize {
        ...
        let start_index = if opcode == OpCode::ConstDyn {
            let x = self.constants.push_data(length.to_vm_byte());
            self.constants.push_data(value);
            x
        } else {
            self.constants.push_data(value)
        };

        self.constants.types.insert(start_index, constant.ctype.clone());

        self.chunk.push_opcode(opcode, line);
        self.chunk.push_operands(start_index.to_vm_byte(), line);
        start_index
    }
    ...
    fn handle_function_data(&mut self, out: &mut StatementHandleResult, data: FunctionData) {
      ...
      //   }
      //   self.current_block -= 1;
      //   out.index = self.chunk.push_opcode(OpCode::BlockOut, line);
    }
    ...
    fn handle_expr(&mut self, expr: Expr, line: usize) -> ExpressionHandleResult {
        ...
        match expr {
            ...
            Expr::Literal(literal) => {
                ...
                let index = self.write_const(literal, line);
