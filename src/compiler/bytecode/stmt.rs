...
use super::{Constants, Context, Env, Patch, PatchKind};
use crate::ir::IRCode;
use types::{Type, Value};

pub fn traverse_statement(env: &mut Env, ctx: &mut Context, ast: &ASTree, statement_id: StmtId) {
...
fn traverse_block(env: &mut Env, ctx: &mut Context, ast: &ASTree, inner: &BlockData) {
    env.deepen_nest();
